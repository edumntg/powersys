...
class PowerSystemSolver_gekko(object):
    ...
    def __construct_optim_model(self, study = 'opf'):
        ...
        m.Obj(self.__optim_objective(study))
        ...
        m.Equations([
            self.__optim__constr_reference_bus_angle(bus) for bus in self.system.buses if bus.reference
        ])
    ...
    def __optim_objective(self, study = 'opf'):
        total_cost = 0.0

        for gen in self.system.generators:
            total_cost += gen.cost(self.Pgen[gen.id])

        return total_cost if study == 'opf' else 1
    ...
    def __optim__constr_reference_bus_angle(self, bus):
